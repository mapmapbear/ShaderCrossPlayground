...
ReflectData::Resource::Resource(const spirv_cross::Resource &other)
    : id(other.id), type_id(other.type_id), base_type_id(other.base_type_id),
      name(std::move(other.name)) {}

static ReflectData getReflectData(const spirv_cross::Compiler &comp,
                                  const spirvbytes &spirvdata) {
  auto rsc = comp.get_shader_resources();
  ReflectData refl{
      .uniform_buffers{rsc.uniform_buffers.data(),
                       rsc.uniform_buffers.begin() +
                           rsc.uniform_buffers.size()},
      .storage_buffers{rsc.storage_buffers.data(),
                       rsc.storage_buffers.begin() +
                           rsc.storage_buffers.size()},
      .stage_inputs{rsc.stage_inputs.data(),
                    rsc.stage_inputs.begin() + rsc.stage_inputs.size()},
      .stage_outputs{rsc.stage_outputs.data(),
                     rsc.stage_outputs.begin() + rsc.stage_outputs.size()},
      .subpass_inputs{rsc.subpass_inputs.data(),
                      rsc.subpass_inputs.begin() + rsc.subpass_inputs.size()},
      .storage_images{rsc.storage_images.data(),
                      rsc.storage_images.begin() + rsc.storage_images.size()},
      .sampled_images{rsc.sampled_images.data(),
                      rsc.sampled_images.begin() + rsc.sampled_images.size()},
      .atomic_counters{rsc.atomic_counters.data(),
                       rsc.atomic_counters.begin() +
                           rsc.atomic_counters.size()},
      .acceleration_structures{rsc.acceleration_structures.data(),
                               rsc.acceleration_structures.begin() +
                                   rsc.acceleration_structures.size()},
      .push_constant_buffers{rsc.push_constant_buffers.data(),
                             rsc.push_constant_buffers.begin() +
                                 rsc.push_constant_buffers.size()},
      .separate_images{rsc.separate_images.data(),
                       rsc.separate_images.begin() +
                           rsc.separate_images.size()},
      .separate_samplers{rsc.separate_samplers.data(),
                         rsc.separate_samplers.begin() +
                             rsc.separate_samplers.size()},
  };
  ...
  SpvReflectResult result = spvReflectCreateShaderModule(
      spirvdata.size() * sizeof(spirvdata[0]), spirvdata.data(), &spvModule);
  if (result != SPV_REFLECT_RESULT_SUCCESS) {
    throw runtime_error(std::string("SPIRV reflection capture failed: ") +
  ...
  const auto sortfn = [](const auto &spvreflvars, auto &inoutscontainer) {
    ...
    std::sort(inoutscontainer.begin(), inoutscontainer.end(),
  ...
  // sort inputs
  {
    ...
    result = spvReflectEnumerateInputVariables(&spvModule, &var_count,
                                               input_vars.data());
  ...
  // sort outputs
  {
    ...
    result = spvReflectEnumerateOutputVariables(&spvModule, &var_count,
                                                output_vars.data());
  ...
  for (int i = 0; i < 3; i++) {
    refl.compute_dim[i] = comp.get_execution_mode_argument(
        spv::ExecutionMode::ExecutionModeLocalSize, i);
  }
...
static void setEntryPoint(T &compiler, const std::string &entrypoint_name) {
  ...
  if (!entryPoints.empty()) {
    compiler.rename_entry_point(entryPoints[0].name, entrypoint_name,
 ...
 * for more info
 * @return instance of DefaultTBuiltInResource struct with appropriate fields
 * set
 */
...
const CompileGLSLResult
CompileGLSL(const std::string_view &source,
            const std::string_view &sourceFileName,
            const EShLanguage ShaderType,
            const std::vector<std::filesystem::path> &includePaths, bool debug,
            bool enableInclude, std::string preamble = "",
            bool performWebGPUModifications = false) {
  ...
  int lengths[] = {int(source.size())};

  const char *names[] = {sourceFileName.data()};

  // set the associated strings
  // shader.setStrings(strings.data(), strings.size());
  const char *sourceArray[] = {source.data()};
  shader.setStrings(sourceArray, 1);
  shader.setStringsWithLengthsAndNames(strings, lengths, names,
                                       std::size(strings));
  ...
  if (performWebGPUModifications) {
    auto remapper = [&]() {
      ...
      // 'trim' adjust ranges
      for (; substr_begin < brace_loc && std::isspace(source[substr_begin]);
           substr_begin++) {
        substr_size--;
      }
      for (;
           substr_size > 0 && std::isspace(source[substr_begin + substr_size]);
           substr_size--) {
      }
      ...
      shader.addBlockStorageOverride(globalUniformBlockName.c_str(),
                                     glslang::TBlockStorageClass::EbsUniform);
    };
    remapper();
  ...
  //=========== vulkan versioning (should alow this to be passed in, or find out
  // from the system) ========

  constexpr int ClientInputSemanticsVersion = 460;
  constexpr glslang::EShTargetClientVersion VulkanClientVersion =
      glslang::EShTargetVulkan_1_3;
  constexpr glslang::EShTargetLanguageVersion TargetVersion =
  ...
  if (!shader.parse(&Resources, ClientInputSemanticsVersion, ECoreProfile,
  ...
  if (debug) {
    spvOptions.emitNonSemanticShaderDebugInfo =
        false; // having these on breaks renderdoc debugging
 ...
 */
// const CompileGLSLResult CompileGLSLFromFile(const CompileTask &task,
//                                             const EShLanguage ShaderType,
//                                             bool debug, bool enableInclude,
//                                             bool noPushConstants,
//                                             std::string preamble = "") {
...
//   std::vector<std::filesystem::path> pathsWithParent(
//       std::move(task.includePaths));
//   pathsWithParent.push_back(task.filename.parent_path());
//   return CompileGLSL(InputGLSL, task.filename.string(), ShaderType,
//                      pathsWithParent, debug, enableInclude, preamble,
//                      noPushConstants);
// }
 ...
 */
IMResult SPIRVToOpenGL(const spirvbytes &bin, const Options &opt,
...
IMResult SPIRVToHLSL(const spirvbytes &bin, const Options &opt,
                     spv::ExecutionModel model) {
...
IMResult SPIRVToDXIL(const spirvbytes &bin, const Options &opt,
                     spv::ExecutionModel model) {
  ...
  auto compileWithNewDxc = [&] {
    ...
    pUtils->CreateBlob(hlsl.sourceData.c_str(), hlsl.sourceData.size(), CP_UTF8,
    ...
    HRESULT result = pCompiler->Compile(
        &sourceBuffer, arguments.data(), arguments.size(), nullptr,
        IID_PPV_ARGS(pCompileResult.GetAddressOf()));

    // Error Handling
    ComPtr<IDxcBlobUtf8> pErrors;
    pCompileResult->GetOutput(DXC_OUT_ERRORS,
                              IID_PPV_ARGS(pErrors.GetAddressOf()), nullptr);
    ...
    pCompileResult->GetOutput(DXC_OUT_PDB,
                              IID_PPV_ARGS(pDebugData.GetAddressOf()),
                              pDebugDataPath.GetAddressOf());
    ...
    pUtils->CreateReflection(&reflectionBuffer,
                             IID_PPV_ARGS(pShaderReflection.GetAddressOf()));

    ComPtr<IDxcBlob> pShaderBinary;
    pCompileResult->GetOutput(
        DXC_OUT_OBJECT, IID_PPV_ARGS(pShaderBinary.GetAddressOf()), nullptr);
    const auto binarySize = pShaderBinary->GetBufferSize();
    hlsl.binaryData = decltype(hlsl.binaryData){
        (char *)pShaderBinary->GetBufferPointer(), binarySize};
  };
#endif
#if defined _MSC_VER
  auto compileWithOldDxc = [&]() {
    ...
    auto result = D3DCompile(
        hlsl.sourceData.data(), hlsl.sourceData.size(), "ST_HLSL.hlsl", nullptr,
        nullptr, "main", profile,
        D3DCOMPILE_DEBUG |
            D3DCOMPILE_SKIP_OPTIMIZATION, // TODO: make this configurable, see
                                          // https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/d3dcompile-constants
                                          // for flags
        0,                                // see
           // https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/d3dcompile-effect-constants
           // for flags
        &code, &errormsg);
    if (result == S_OK) {
      hlsl.binaryData = decltype(hlsl.binaryData){
          (char *)code->GetBufferPointer(), code->GetBufferSize()};
    } else {
...
IMResult SPIRVtoMSL(const spirvbytes &bin, const Options &opt,
                    spv::ExecutionModel model) {
  ...
  options.platform = opt.mobile
                         ? spirv_cross::CompilerMSL::Options::Platform::iOS
                         : spirv_cross::CompilerMSL::Options::Platform::macOS;

  // order textures / samplers by binding order, not by order of first use
  options.enable_decoration_binding = true;

  // don't enable this. It will set everything to be an argument buffer.
  // options.argument_buffers = true;
  options.argument_buffers_tier =
      spirv_cross::CompilerMSL::Options::ArgumentBuffersTier::Tier2;
  msl.set_msl_options(options);

  constexpr auto rgl2sc = [](Options::BindlessSettings::Type type)
      -> spirv_cross::SPIRType::BaseType {
    switch (type) {
    case decltype(type)::SampledImage:
      return spirv_cross::SPIRType::SampledImage;
    case decltype(type)::Buffer:
  ...
  constexpr auto resourceRenumber = [](auto &&res, auto &&opt,
    ...
    while (itr < res.size()) {
      ...
      if (value > 31) {
        throw std::runtime_error("Adjusted index too large: " +
                                 std::to_string(value));
      }
  ...
  if (model == spv::ExecutionModel::ExecutionModelVertex &&
      opt.bufferBindingSettings.stageInputSize > 0) {
  ...
  }
  {
    ...
    resourceRenumber(res, opt, "[[sampler(",
                     [&opt, &samplerIdx](auto value) { return samplerIdx++; });
  }

  return {std::move(res), "", std::move(refldata)};
}

IMResult SPIRVToWGSL(const spirvbytes &bin, const Options &opt,
                     spv::ExecutionModel model) {
#if ST_ENABLE_WGSL
  tintInitMtx.lock();
  if (!tintInit) {
  ...
  auto tintprogram =
      tint::reader::spirv::Parse(bin, {.allow_non_uniform_derivatives = true});
  ...
  throw std::runtime_error(
      "ShaderTranspiler was not compiled with WGSL output support");
#endif
}

#if __APPLE__
extern IMResult SPIRVtoMBL(const spirvbytes &bin, const Options &opt,
                           spv::ExecutionModel model);
#endif
/**
...
std::vector<uint32_t> ReadSpirvFile(const std::string &filename) {
  ...
  if (size % sizeof(uint32_t) != 0) {
    throw std::runtime_error(
        "Invalid SPIR-V file size: not a multiple of 4 bytes");
  }
  size_t word_count = size / sizeof(uint32_t);
  std::vector<uint32_t> spirv_data(word_count);

  if (!file.read(reinterpret_cast<char *>(spirv_data.data()), size)) {
    throw std::runtime_error("Failed to read SPIR-V file");
  }
  return spirv_data;
}

static CompileResult CompileSpirVTo(const spirvbytes &spirv, TargetAPI api,
                                    const Options &opt, APIConversion types) {
  switch (api) {
...
CompileResult ShaderTranspiler::Compile(const CompileTask &task, TargetAPI api,
                                        Options &opt) const {
  if (opt.version == 0) {
    switch (api) {
    case TargetAPI::OpenGL:
  ...
  CompileGLSLResult spirv = {};
  spirv.spirvdata = ReadSpirvFile(task.sourceFileName.c_str());
  auto compres = CompileSpirVTo(spirv.spirvdata, api, opt, types);
  // TODO(parse uniform && attrib)
